---
title: "Thin to Thick: The Hidden Design Challenge in Every AI Tool"
date: 2025-12-15
tags:
  - ai
  - design
...
Scroll through LinkedIn and you'll find practitioners mapping their journey. Stage 1: casual use, random questions. Stage 2: power user, saved prompts, custom instructions. Stage 3: packager, building reusable workflows. Stage 4: chaos, too many tools that don't talk to each other. Stage 5: workspace need, craving a single place where everything lives.
...
**Skills** are the most interesting primitive because they've exploded beyond their original conception. At base, a skill activates automatically based on context. When you say "write a blog post," the relevant skill loads without you asking. Skills are markdown files that can reference other files, accumulating resources and examples. The assumption: users want "always-on" expertise that doesn't require explicit invocation. The skill should know when it's needed.

But skills have grown. A skill directory can now contain not just markdown instructions but executable scripts: Python formatters that run after every file edit, shell scripts that validate outputs, automation that fires when the skill activates. The skill becomes a container for behavior, not just knowledge. It can hold reference documents, example files, configuration, and now code that executes. The boundary between "instructions for the model" and "automation that runs alongside the model" has blurred. A sufficiently developed skill starts to resemble an application more than a prompt.

This expansion wasn't in the original design. Users discovered they needed more than markdown; Anthropic accommodated; the primitive grew. The same pattern is visible across the Claude Code ecosystem: primitives that started simple have accreted capability as users pushed against their limits. What was thin became thick, not through intentional design but through accumulated extension.

**Commands** are explicit. You type `/deploy` or `/review` and something happens. Commands are saved prompts, shortcuts for repeated tasks. The assumption: some behaviors should only happen when requested. The user stays in control of when.

Commands now live inside skills, nested in a `workflows/` subdirectory. A skill can contain multiple commands, each a specialized operation within the skill's domain. The taxonomy that started as six distinct categories has developed internal structure: skills contain commands, commands reference resources, resources include scripts. The flatness of the original design has grown hierarchical as users built more complex configurations.
...
But plugins also reveal the cost of taxonomic complexity. A plugin contains skills, which contain commands, which reference resources, which include scripts, alongside hooks and agent configurations and MCP setups. The distribution problem is solved, but the comprehension problem remains. What does this plugin actually do? The answer requires understanding six primitive types and their nesting relationships.
...
Intuitive nesting matters too. A skill contains commands, not the reverse. The hierarchy should follow how users think about capability and specificity, not how engineers think about code organization. When the mental model matches the implementation model, composition becomes learnable.
