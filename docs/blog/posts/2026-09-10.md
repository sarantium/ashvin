...
So W should be set as a limit, not tracked as a score. A limit means: no operator reviews more than a fixed number of artifacts a day, no operator carries more than a fixed number of review hours a week, and the person who sets those numbers is not the person responsible for output. A W score tells the team what the harness did to its operators. A W limit stops the harness from doing it.
...
|---|---|---|---|
| **Q** Question | Is it testable and worth asking | Time from signal to question | $ to form it |
| **A** Answer | Does it pass review | Time from question to verified artifact | $ in tokens, compute, and review hours |
| **U** Understanding | Can the person explain it and catch it when wrong | Time from artifact to that point | $ in hours spent understanding it |
| **W** Wellbeing | Operator's own rating; multi-turn evals for users | Review time per artifact; hours beyond contract | $ in turnover and lost hours |
