...
Scope composes upward. A Pack handling untrusted input — a counterparty's filing, a citizen submission, an inbound document of unknown provenance — can split tool access across subagents within itself: a reader holds permissions to read but not write, an orchestrator holds neither, a writer holds Write but never opens the outsider's file. The threat model is declared per-Pack rather than per-item. An injection that lands in the reader's context cannot reach a tool that produces a side-effect, because the writer that holds those tools never reads the outsider's input. The substrate property is unchanged — declared scope, enforced at injection — but the unit shifts from item to Pack-shape.
