...
date: 2026-04-09
description: "A composable architecture for agent systems: a production pipe measured by TTQ, TTA, and TTU, ten building blocks that form a search space, and an outer loop that recomposes them against a fitness function."
tags:
  - ai
...
Each stage is a hill-climb against its gate. The diagram suggests linear flow — TTQ produces a question, G1 checks it, TTA produces an artefact, G2 checks it — but in practice each transition is iterative. TTQ refines the question until G1 passes; TTA refines the artefact until G2 passes; understanding deepens until G3 passes. The gate isn't a checkpoint at the end of a stage. It's the fitness signal that drives the stage's inner loop. This is why external gates matter beyond correctness: they supply the gradient the loop hill-climbs against. Self-evaluation collapses the gradient and the loop with it. Stage and gate are paired; either alone is incomplete.
...
A building block is present or absent without breaking the pipe, configurable on its own, and domain-agnostic at the pattern level. Ten have emerged so far, in three roles.
...
**Blocks that govern behavior.** Constraints define what the pipe will not do: enforced dependency directions, rigid layering, a novelist's rule that certain themes stay out. Some constraints cascade into architecture: a one-minute build time ceiling, enforced as an invariant, drives the codebase toward finer-grained module boundaries when the build breaches the threshold, and structural quality improves as a side effect. Gate Criteria define what "good enough" means at each transition. The gate mechanism is invariant; the criteria are domain-specific. The separation is load-bearing: agents cannot reliably evaluate their own work. Left to self-evaluate, they rewrite test suites to match their code or satisfy assertions with `return true`. Gates must be architecturally external, enforced by the system rather than trusted to the agent. Practitioners building evaluation infrastructure at scale have found that error analysis (the careful work of studying where and why gates fail) ends up consuming the majority of real development effort, often dwarfing the time spent on the pipe itself. Building the thing that tells you whether the pipe is working turns out to be harder than building the pipe.

Skills encode active procedures the agent invokes when it recognizes a matching situation. A coding harness skill carries the full procedure for landing a pull request: invoke CI, wait for review, handle conflicts, merge. A novelist's skill: when a chapter exceeds four thousand words, split at the natural scene break and check pacing in both halves. Constraints say what the pipe will not do; Gate Criteria say what good enough looks like. Skills say how to do it well. They are the block most directly refined by the outer loop, tightened with each cycle by the delta between what agents produced and what was needed.
...
One composition pattern substitutes redundancy for monitoring: instead of running one pipe under close supervision, run several in parallel with tight specifications and select the best result. The human's role shifts from real-time supervision to post-hoc selection, a composition-level expression of the maturity shift from Early to Late.

![The ten building blocks](../blog_assets/harness_building_blocks.png)
*The building blocks: ten composable elements in three roles. Present or absent without breaking the pipe.*

These ten are not a closed inventory. They are a search space, open by design, with an admission criterion (composable, configurable, applicable across domains) that tests new candidates. Different compositions produce different harnesses. An algorithmic trading harness composes Context Files, Constraints, Gate Criteria, Diagnostic History, and Pipe Composition, with no Human in the Loop and no Wiki. A research harness composes all ten. Ten blocks with binary presence-or-absence yield 1024 possible compositions, each carrying its own configuration space. Fixed blocks with no composition produce a single harness design; the outer loop would have nothing to search. Composability gives the pattern a space to explore.
...
A software team collecting session logs and review feedback into shared storage, running nightly analysis to surface patterns, and feeding refinements back into Skills and documentation operates the same mechanism: the input is execution data from the day's work, the output is tighter procedures for tomorrow's.

The outer loop operates at two granularities. At the block level, it adds, removes, or reconnects blocks on a slow cadence. Within blocks, it refines content: sharpening skill definitions, tightening gate criteria, enriching context files based on the delta between what agents produced and what the diagnostic history shows was needed. Most daily improvement happens at this second level. The block topology stays constant; the content within blocks improves continuously. One selects which instruments are in the orchestra; the other is the musicians practicing.
...
The outer loop is itself a hill-climb at a different scale. Inner gates supply per-stage gradients within a run; the fitness function supplies the gradient across runs. The mechanism is the same; the scale and the gradient shape differ.
...
The resolution has a corollary for block design. Blocks that work within the model's natural behavior survive model generations: context files as prose, constraints as tests, skills as structured markdown. The model already knows how to process these formats. Blocks that fight the model through external scaffolding (forced state machines, custom DSLs, adversarial wrappers) are built to compensate for what the model cannot do, and break when the next model can. Reinforcement learning calls this the on-policy/off-policy distinction: an on-policy harness encodes constraints in formats the model natively processes; an off-policy harness encodes workarounds, and workarounds have a shelf life. Practitioners and platform engineers have converged on this independently: guards built from prose, tests, and skills survive model transitions; external scaffolding does not.
...
A team does not sit at one level. In the same week, the same group may operate at Mature for production iteration, Middle for established features, and Early for zero-to-one work; maturity is a portfolio across domains, not a position on a ladder.
...
The maturity model is a lens: where is the human now, and is that where they should be? If the human is doing gate evaluation on a pipe with a 98% pass rate, the answer is no. If the human is at the Seed but cannot explain what the pipe produces, TTU has failed through both channels, and the human needs to move back in.

Regression is not always failure. A model release that changes agent behavior can invalidate assumptions the harness depends on, pulling a domain from Late back to Early until it adapts. A novel problem domain forces humans back into the pipe regardless of how mature production workflows are. A deliberate investment in infrastructure, accepting slower output now to enable a higher ceiling later, is a chosen regression. The maturity model is bidirectional; the outer loop should detect regression early, not to prevent it, but to distinguish planned regression from unplanned and respond accordingly.

But how do you know what the system has actually captured versus what still lives only in someone's head?
...
The test also reveals what can be shipped. A specification can be refined iteratively: write it, implement from it alone, compare to the original, tighten, repeat. When an independent agent with no access to the original codebase can reproduce the system from the specification alone, the result is a distribution format, lighter than source code, carrying purpose and structure without implementation detail. The Seed becomes a portable artifact.
...
The phoenix test need not be a periodic diagnostic. It can operate as a runtime workflow. When an agent produces work that fails review, the system trashes the result and starts from scratch — a micro-phoenix-test on every failed trajectory. The restart is cheap; code is disposable. What makes it valuable is the diagnostic trigger: before restarting, the system asks what caused the failure and fixes the root cause in the Seed, the skills, or the constraints. Each burn-and-restart cycle tightens the harness. The willingness to restart rather than patch is itself a maturity signal: at early maturity, you fix the failing output; at late maturity, you fix the system that produced it.
...
Ten building blocks have been discovered so far; more exist. The outer loop explores the space. What the pattern provides is the architecture for that exploration, and the test of whether your system actually knows what you think it knows.
