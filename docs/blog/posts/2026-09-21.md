...
date: 2026-09-21
description: "AI cost is six numbers multiplied together. A checklist organised around the four you can shrink — and the two you should be growing."
tags:
  - ai
  - applied-ai
  - economics
...
.md-typeset .section-head .effort {
  margin-left: auto;
  font-family: "Roboto Mono", monospace;
  font-size: 0.7em;
  font-weight: 500;
...
<div class="act-head" markdown>
Act 1 — See the numbers <span class="act-note">You cannot shrink a term you cannot read. Everything in Act 2 assumes this exists.</span>
</div>

<div class="section-head" markdown>
<span class="num">01</span> <span class="name">Meter Every Term</span> <span class="effort">Low effort</span>
</div>
...
- [ ] **Cache hit rate measured per tool** — this is the number you will act on in section 06
...
<div class="section-head" markdown>
<span class="num">02</span> <span class="name">Put the Number in Front of the Person Spending It</span> <span class="effort">Low effort</span>
</div>
...
<div class="section-head" markdown>
<span class="num">03</span> <span class="name">Benchmark Before You Change Anything</span> <span class="effort">Medium effort</span>
</div>
...
<div class="section-head" markdown>
<span class="num">04</span> <span class="name">Read Other People's Bills</span> <span class="effort">Low effort</span>
</div>

<div class="section-context" markdown>
Almost nothing in Act 2 was discovered here. Uber published that defaulting subagents to a cheaper model was the single most impactful change it made; Spotify published the 350-line threshold and, more usefully, the list of things not to delegate. Each of those cost someone a quarter to learn and costs you an afternoon to copy. Most of the experiments you are about to fund have already been run somewhere.
</div>

- [ ] **A named list of who is ahead of you, and on which term** — not "companies doing AI well". Who has solved caching, who has solved routing, who has renegotiated at your volume
- [ ] **Published engineering write-ups mined for defaults, not for inspiration** — the transferable artifacts are thresholds, TTLs, percentages and named anti-patterns. A post you finish nodding at and change nothing about was entertainment
- [ ] **The plumbing copied across industries, the denominator not** — metering, caching, gateways and routing are industry-agnostic, and the organisations furthest ahead on them are often nothing like yours. The unit of work does not travel: cost per merged PR is a software factory's denominator, and a bank's or an insurer's is a different one. Borrow the mechanism, derive your own unit
- [ ] **One conversation a quarter with someone operating at your scale** — what never gets published is the failures, the actual vendor terms, and the size of the team it took. Those three decide whether you should attempt a thing at all, and none of them are in the blog post
- [ ] **Borrowed numbers treated as hypotheses, not settings** — every figure in this checklist came out of someone else's workload mix. It goes through section 03 before it becomes anyone's default
- [ ] *(when applicable)* Something of your own to trade — see section 12. Nobody discusses their real discount with someone who will not discuss theirs

<div class="act-head" markdown>
Act 2 — Shrink the four terms <span class="act-note">One section per term. Defaults do the heaviest lifting here, because a default applies to every session, every user, indefinitely, without anyone agreeing to it.</span>
</div>

<div class="section-head" markdown>
<span class="num">05</span> <span class="name">First, Delete What Should Not Be in the Equation</span> <span class="effort">Low effort</span>
</div>
...
<div class="section-head" markdown>
<span class="num">06</span> <span class="name">Tokens per Request</span> <span class="effort">Low effort</span>
</div>
...
<div class="section-head" markdown>
<span class="num">07</span> <span class="name">Requests per Turn</span> <span class="effort">Medium effort</span>
</div>

<div class="section-context" markdown>
Most of what a coding agent does is not thinking. It is reading files, and emitting output predictable from the files next to it. Both can leave the expensive context entirely — Spotify measured mean savings of around 90% on bulk reads across a Java monorepo by routing them to a cheap worker model. See also section 08: grounding cuts requests per turn as well as turns per session.
</div>
...
<div class="section-head" markdown>
<span class="num">08</span> <span class="name">Turns per Session</span> <span class="effort">Medium effort</span>
</div>
...
<div class="section-head" markdown>
<span class="num">09</span> <span class="name">Price per Token: Run a Cheaper Model</span> <span class="effort">Medium effort</span>
</div>
...
<div class="section-head" markdown>
<span class="num">10</span> <span class="name">Price per Token: Pay Less for the Same Model</span> <span class="effort">Low to high effort</span>
</div>
...
- [ ] *(when applicable)* A volume threshold set before self-hosting is even considered <span class="cond">**(high effort)**</span> — self-serving weights becomes defensible somewhere in the low millions of tokens per day. Below that you are buying an on-call rotation, not a saving
- [ ] *(when applicable)* Throughput per GPU treated as the metric, not GPU count <span class="cond">**(high effort)**</span> — batching, quantisation and serving-stack choices move this several-fold, and none of it matters unless you self-host
- [ ] *(when applicable)* Spot or multi-cloud capacity for interruptible workloads <span class="cond">**(high effort)**</span> — cheaper per GPU-hour, and it requires a workload that tolerates eviction

<div class="act-head" markdown>
Act 3 — Own the plumbing, then sell it <span class="act-note">What makes Act 2 executable, and what it turns into once it works.</span>
</div>

<div class="section-head" markdown>
<span class="num">11</span> <span class="name">Own the Switchboard</span> <span class="effort">Medium effort</span>
</div>

<div class="section-context" markdown>
Routing you cannot change is not routing. One layer between your agents and every provider is what makes sections 09 and 10 executable, and it is also where you either take on lock-in or shed it.
</div>
...
<div class="section-head" markdown>
<span class="num">12</span> <span class="name">Sell the Control System</span> <span class="effort">High effort</span>
</div>
...
> **The loop.** Cost control has no finish line, because both inputs to the equation move continuously: your workload mix, and the vendors' prices. Every model release resets the frontier in section 03, which invalidates the routing in section 09. Every jump in adoption changes which of the four terms now dominates, and therefore which section is worth your quarter. And the published state of the art moves on a schedule of its own — re-read the field when you re-run the benchmark. Go back to the benchmark, not to the spreadsheet — re-measure the terms and let them tell you where to work next.
...
**Provider pricing and prompt-caching documentation (Anthropic, OpenAI).** The multipliers behind section 06 — cache reads at 0.1x the standard input rate, 5-minute cache writes at 1.25x and 1-hour writes at 2x, with TTL options of 5 minutes and 1 hour from Anthropic and 30 minutes from OpenAI — and the roughly half-price batch endpoints behind section 05. Summarised in the Uber post; check current published pricing before modelling any of it.
